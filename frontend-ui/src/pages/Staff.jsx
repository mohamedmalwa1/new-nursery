...
export default function Staff() {
  const [rows,setRows]=useState([]),[load,setLd]=useState(true);
  const [drawer,setDr]=useState(false),[edit,setEdt]=useState(null);

  const fetchRows=()=>{setLd(true);
    fetch("http://127.0.0.1:8000/api/staff/")
      .then(r=>r.json()).then(d=>{setRows(d);setLd(false);})
      .catch(()=>{toast.error("Load failed");setLd(false);});};
  useEffect(fetchRows,[]);

  const del=id=>fetch(`http://127.0.0.1:8000/api/staff/${id}/`,{method:"DELETE"})
      .then(r=>{if(!r.ok)throw 0;toast.success("Deleted");fetchRows();})
  ...
  return (
    <div>
      ...
      {load? <p className="p-4">Loading…</p>
