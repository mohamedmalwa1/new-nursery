...
export default function AttendanceForm({ initial, onSaved, onCancel }) {
  const [students,setStudents]=useState([]); const[staff,setStaff]=useState([]);
  ...
  const[form,setForm]=useState({
    student:"", staff:"", date:"", status:"PRESENT",
    check_in:"", check_out:"", notes:"", ...initial,
  });
      ...
      <div className="grid grid-cols-2 gap-4">
        <L label="Student">
          <select value={form.student||""}
                  onChange={e=>update("student",e.target.value||null)}
